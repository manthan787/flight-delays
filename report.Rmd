...
We plot the top 5 most active airports and airlines below along with the number of flights as the measure of activity. We observe that _Delta Airlines_ is the most active airline and _LAX_ is the most active airport.
...
grid.arrange(top_5_airlines.plot, top_5_airports.plot, ncol=2, widths= c(6, 6), heights=c(4, 4))
```
...
ggplot(agg, aes(y=airportID, x=airlineID)) + geom_tile(aes(fill=mDelay), color="white") + scale_fill_gradient(low = "white", high = "red") + labs(x = "Airlines",y="Airport",col = "Delay in Minutes", title="Fig 1. Mean delays from all airlines to top 5 airports across all years") + theme_light() + coord_fixed(ratio = 1) + theme(axis.text.x = element_text(angle = 90))
