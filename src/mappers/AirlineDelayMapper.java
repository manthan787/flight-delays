 ...
 * Forms a custom key from Airline, Airport, month and year.
 * Writes the custom key and delay(value) to output.
 */

public class AirlineDelayMapper extends Mapper<LongWritable, CleanDataWritable, Text, FloatWritable> {

    @Override
    protected void map(LongWritable key, CleanDataWritable value, Context context)
            throws IOException, InterruptedException {
        IntWritable airlineID = value.getAirlineID();
        ...
        String outKey = airlineID.toString() + "," + airportID.toString() + ","
