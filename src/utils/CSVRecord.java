...
/**
 * Parse one line of text containing comma separated values. We support fields that either sequences of character between
 * delimiters (commas, start and end of line), or between quotes. This class has several known limitations, it does not support
 * escaped quotes and has a hard bound on the number of fields. Performance-wise, we could speed it up the parsing by avoiding the
 * construction by reusing instances for multiple lines. As parsing does not appear to be the bottle neck, it may not be worth the
 * effort.
 *
 * @author Jan Vitek
 *
...
public class CSVRecord implements Iterable<String> {
	...
	public CSVRecord(String line) {
		...
		for (i = 0; i < line.length(); i++) {
			...
			if (c == Q) {
				if (!inString) {
					if (inputSeen) throw new Error("CSV Maformed, multiple elements in a field " + line);
					inputSeen = false;
					inString = true;
					start = (short) (i + 1);
				} else { // inString
